...
FDawnShaderConductorLoader::~FDawnShaderConductorLoader()
{
	...
	// safe to unload out from under a still-running process; matches
	// FShaderConductorContext::Shutdown()'s Linux-only explicit-shutdown
	// comment in Epic's own ShaderConductorContext.cpp (mutex left dangling
	// on exit is called out there as an accepted Linux quirk).
}
