...
## Update 16: the FULL UnrealTournament game target compiles + links to wasm (not just the DawnRHITest program) and instantiates in-browser

**Milestone:** the actual game — `UnrealTournament` (TargetType.Game: full Engine + CoreUObject + Slate + UMG + the UT game modules), not the bare `DawnRHITest` program — now builds to a WebGPU wasm binary with our open DawnRHI + clean platform layer, and its Emscripten runtime instantiates in a real headless browser.

### What was changed (all verified against the build/link log)
1. **`UnrealTournament.Target.cs` — SimplyStream platform block** (snapshot: `engine-patches/ut-wasm-fullgame/UnrealTournament.Target.cs`): `bUsePCHFiles=true` (UT4 is UE4.15-era, not IWYU-clean → needs SharedPCH.Engine, per RECIPE.md), `bCompileCEF3=false` (no wasm CEF3), `AdditionalCompilerArguments += "-Wno-error -Wno-unused-template --use-port=emdawnwebgpu"`.
2. **`SimplyStreamToolChain.cs` — broadened the link-flag gate** (patch: `engine-patches/ut-wasm-fullgame/ut-wasm-SimplyStreamToolChain.cs.patch`): the DawnRHITest-only injection block (PROXY_TO_PTHREAD, ASYNCIFY, ICU preload, --use-port) now also matches `UnrealTournament`, because target link args are dropped by this toolchain. **BUT** `--use-port=emdawnwebgpu` was made DawnRHITest-only again (see wall 1 below).
3. **`SimplyStreamPlatformFile.cpp` + `SimplyStreamPlatformBootShims.cpp` — WITH_ENGINE guards** (snapshots in `engine-patches/ut-wasm-fullgame/`): our Update-8 clean-room `GetPlatformPhysical()` / `IsUsingLowCoreCount()` shims were needed only for the bare DawnRHITest (`bCompileAgainstEngine=false` → WITH_ENGINE=0). The full game Core defines both itself → guarded with `#if !WITH_ENGINE` to avoid duplicate symbols (see wall 2).

### Walls cleared this run (each with real link-log evidence)
- **All 909 compile actions succeeded** — the entire UT4 game + engine compiled to wasm with zero errors on the first pass (even WebBrowser.cpp; CEF3 disabled).
- **Wall 1 — emdawnwebgpu JS double-include.** Link failed: `library_webgpu_generated_struct_info.js ... Identifier 'structInfo32' has already been declared`. Root cause: the full-game SimplyStream link *already* carries the vendored (open, upstream Google) emdawnwebgpu JS glue (`Source/ThirdParty/emdawn/Private/library_webgpu_*.js`) + `emdawn`/`webgpu_utils` objects, so our stock `--use-port=emdawnwebgpu` added a *second* copy. Fix: `--use-port` is DawnRHITest-only again; the full game uses the vendored emdawnwebgpu already on its link. (Clean-room note: the vendored emdawn is open Google emdawnwebgpu, NOT the closed WebGPURHI — safe to use. **Open ABI caveat:** DawnRHI still *compiles* against the stock port's webgpu.h; it now *links* against the vendored bridge. Both are emdawnwebgpu; if a struct layout skews at runtime, the fallback is to instead exclude the vendored emdawn and keep stock end-to-end.)
- **Wall 2 — duplicate platform-file symbols.** Next link error: `duplicate symbol: IPlatformFile::GetPlatformPhysical()` and `IsUsingLowCoreCount()` — our Update-8 shims vs. the full game Core's own. Fix: `#if !WITH_ENGINE` guards (above).
- **Result: `Result: Succeeded`** — `UnrealTournament.{wasm(673MB),js(1.3MB),data(22MB ICU),html}` in `UnrealTournament/Binaries/SimplyStream/`. Only benign `warning: undefined symbol: __emscripten_atomics_sleep` (provided by the generated .js at runtime, per RECIPE).

### In-browser boot (furthest point this run)
Booted via `host/ut-shim.html` + `host/serve.py` + `tools/cdp_capture.mjs` (new `tools/ut_boot_launch.sh` wraps it). Env notes: the cached Playwright Chrome needs the nix `LD_LIBRARY_PATH` (`/tmp/chrome_ld_library_path.txt`) AND `NO_ANGLE_VULKAN=1` (drops `--use-angle=vulkan`, which fails on this driverless headless box). Furthest observed: the UnrealTournament wasm **instantiates** (`Used memory before allocating anything was 46.79MB` — Emscripten runtime start) and Dawn/WebGPU sets up an adapter (limit-reduction messages). **No UE engine log line (LogInit/PreInit) captured within the 240s window** — the 673MB memory64 wasm consumes most of that window just compiling in headless Chrome, and the box has no real Vulkan driver (SwiftShader fallback).

### Ranked next steps toward the main menu
